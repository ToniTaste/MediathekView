...
public class GuiFilmeModelHelper {
  ...
  private void performTableFiltering() {
    ...
    if (dontShowAbos) stream = stream.filter(film -> film.getAboName().isEmpty());
    ...
    stream.filter(Objects::nonNull).forEachOrdered(this::addFilmToTableModel);
    stream.close();

    if (dontShowSeen) historyController.emptyMemoryCache();
  }
  ...
  public TableModel getFilteredTableModel() {
    if (!listeFilme.isEmpty()) fillTableModel();
    return filmModel;
  }
